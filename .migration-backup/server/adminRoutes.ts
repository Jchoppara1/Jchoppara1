...
export function registerAdminRoutes(app: Express) {
  ...
  app.get("/api/admin/wines", requireAdmin, async (_req: Request, res: Response) => {
  ...
  app.post("/api/admin/wines", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.patch("/api/admin/wines/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      const listType = (req.query.list as string) === "glass" ? "glass" : "bottle";
  ...
  app.patch("/api/admin/wines/:id/out-of-stock", requireAdmin, async (req: Request, res: Response) => {
    try {
      const listType = (req.query.list as string) === "glass" ? "glass" : "bottle";
  ...
  app.patch("/api/admin/wines/:id/labels", requireAdmin, async (req: Request, res: Response) => {
    try {
      const listType = (req.query.list as string) === "glass" ? "glass" : "bottle";
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: "Invalid labels", details: error.errors });
      }
      res.status(500).json({ error: "Failed to update labels" });
    }
  });

  app.get("/api/admin/menu", requireAdmin, async (_req: Request, res: Response) => {
  ...
  app.patch("/api/admin/menu/:id/out-of-stock", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { outOfStock } = outOfStockSchema.parse(req.body);
  ...
  app.patch("/api/admin/menu/:id/labels", requireAdmin, async (req: Request, res: Response) => {
    try {
