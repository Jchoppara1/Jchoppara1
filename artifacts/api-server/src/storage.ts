...
function findDataRoot(): string {
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(path.join(candidate, 'delbarcsv'))) return candidate;
  }
  return process.cwd();
}
...
function loadWinesFromCSV(): WineWithClassification[] {
  const csvPath = path.join(DATA_ROOT, 'delbarcsv', 'wine_list.csv');
...
function loadFoodsFromCSV(): Omit<InsertFood, "id">[] {
  const csvPath = path.join(DATA_ROOT, 'delbarcsv', 'food_menu.csv');
